...
# Create flatpak-node-generator.py if it doesn't exist
if [ ! -f "flatpak-node-generator.py" ]; then
    echo -e "${YELLOW}Downloading flatpak-node-generator...${NC}"
    curl -O https://raw.githubusercontent.com/flatpak/flatpak-builder-tools/master/node/flatpak-node-generator.py
    chmod +x flatpak-node-generator.py
fi
...
python3 ../flatpak-node-generator.py npm package-lock.json -o ../flowforge-electron-sources.json
cd ..
rm -rf electron-temp
...
cat > io.github.flowforge.FlowForge.devel.yml << 'EOF'
...
echo "  ./build-flatpak.sh      - Build the Flatpak"
echo "  ./create-bundle.sh      - Create distributable .flatpak file"
echo "  ./setup-config.sh       - Configure API keys after installation"
echo ""
